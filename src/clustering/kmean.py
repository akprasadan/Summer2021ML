...
class KCluster(Clustering):
    ...
    def compute_distance_matrix(features, centers):
        ...
        distance_matrix = np.linalg.norm(pairwise_differences, axis = -1)
    ...
    def update_clusters(features, centers):
        ...
        for row in range(n):
            ...
            closest_center_indices = np.where(distance_to_centers == np.amin(distance_to_centers))[0]
            
            if row in closest_center_indices:
                closest_center_idx[row] = row

            else: 
                closest_center_idx[row] = closest_center_indices[0]
    ...
    def update_means(features, closest_center_idx, k):
        ...
        for cluster_index in range(k):
            ...
            if cluster_subset.size == 0:
                cluster_means[cluster_index, :] = np.random.rand(1, d)
            else:
    ...
    def fit(self):
        ...
        initial_data_idx = np.random.choice(self.sample_size, self.k)
        ...
        while error > self.threshold:
            total_steps += 1
            updated_indices = KCluster.update_clusters(self.features, initial_means)
            updated_means = KCluster.update_means(self.features, updated_indices, self.k)
        ...
        feature_means = np.zeros((self.sample_size, self.dimension))

        for i in range(self.sample_size):
            # The ith prediction is the jth row of the means, 
            # where j = predicted_clusters[i]
            feature_means[i] = self.final_centers[updated_indices[i], :]
